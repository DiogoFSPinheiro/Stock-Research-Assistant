...
from xtb_trading_bot.config import AppConfig, RiskConfig, TelegramConfig, UniverseConfig, XtbConfig
...
class TradingBotTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.config = AppConfig(
            xtb=XtbConfig("demo", "user", "pass", "host", 5124, True),
        ...
        self.client = XtbClient(self.config.xtb, ["EURUSD", "AAPL"], ["SPX500"])
        self.approvals = TelegramApprovalService(
            self.config.telegram,
            self.config.risk.signal_expiry_minutes,
            http_post=lambda url, payload: None,
        )
        self.approvals.positions_provider = self.client.list_positions
        self.bot = TradingBot(
            config=self.config,
    ...
    def test_rejected_signal_does_not_submit_order(self) -> None:
        self.bot.scan()
        proposal = self.state_store.list_pending_proposals()[0]
